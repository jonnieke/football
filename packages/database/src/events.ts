...
export class SourceEventHeldError extends Error {
  constructor(public readonly reviewId: string) {
    super("Source event is held by an operator review");
    this.name = "SourceEventHeldError";
  }
}

/** Caller owns transaction, including rollback on any outbox failure. */
export async function saveCanonicalEvent(
  ...
  if (
    ...
    await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${`source-review:${event.fixtureId}`}, 0))`;
    const review = await tx.sourceEventReview.findUnique({
      where: {
    ...
    if (review !== null && review.status !== "published")
      throw new SourceEventHeldError(review.id);
  }
...
export async function createCanonicalEvent(
  ...
  assertInternalEventIds(event);
  try {
    return await prisma.$transaction((tx) => saveCanonicalEvent(tx, event));
  } catch (error) {
    if (
      typeof error !== "object" ||
      error === null ||
    ...
    return prisma.$transaction((tx) => saveCanonicalEvent(tx, event, error));
  }
}
